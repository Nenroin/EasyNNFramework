...
class CustomSequentialModel(SequentialModel):
    def __init__(self,
                 ...
                 metric: Metric = average_mse_loss_metric(),
                 ):
        super().__init__(
            layers=layers,
            optimizer=optimizer,
    ...
    def train_epoch(
        ...
        for i, (x_batch, e_batch) in enumerate(tqdm(train_data,
    ...
    def test_epoch(self, test_data: DataBatchWrapper, disable_tqdm = False):
        if len(test_data) == 0:
            return

        self.metric.reset_state()
        for i, (x_batch, e_batch) in enumerate(tqdm(test_data,
                                                    total=len(test_data),
                                                    disable=disable_tqdm)):
