...
def scaling_pil(scale_range: (float, float) = (0.5, 1.5), copies: int = 2, padding_value: float = 0.):
    def augment_function(data_tuple):
...
def rotating(rotation_range: (float, float) = (-30, 30), copies: int = 2, padding_value: float = 0.):
    def augment_function(data_tuple):
        img, label = np.array(data_tuple[0]), data_tuple[1]
